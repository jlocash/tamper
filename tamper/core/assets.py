...
def load_asset_from_file(g: Graph, asset_file: PathLike[str]) -> MediaAsset:
    ...
    asset = MediaAsset.new(g, asset_uri)
    asset.media_type = media_type
    asset.checksum = "sha256:" + checksum
    asset.file_path = str(Path(asset_file).absolute())

    if media_type.startswith("image/"):
        asset = ImageAsset.new(g, asset_uri)
        _extract_image_metadata(asset)
    elif media_type.startswith("audio/"):
        asset = AudioAsset.new(g, asset_uri)
        _extract_container_metadata(asset)
        return AudioAsset(g, asset_uri)
    elif media_type.startswith("video/"):
        asset = VideoAsset.new(g, asset_uri)
        _extract_container_metadata(asset)
